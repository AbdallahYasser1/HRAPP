...
$request['date']=date('Y-m-d');
        ...
        if(!in_array($mission->requests()->first()->status, ['approved','closed','in-progress'])||(strtotime($mission->requests()->first()->end_date)<strtotime($request['date'])||strtotime($mission->requests()->first()->start_date)>strtotime($request['date']))||$mission->requests()->first()->user_id!=Auth::id()||$mission->paid){
